...
import { AgendaRepository, SelectAgenda, SelectAgendaBooking, SelectAgendaBookingQuery, SelectAgendaQuery, SelectAgendaRecurrence, SelectAgendaRecurrenceQuery } from '../agenda';
...
import { AgendaWithAgendaBooking, AgendaWithClass, AgendaWithCoach, AgendaWithLocation, AgendaWithParticipant } from '#dep/service/agenda';
...
export class KyselyMySqlAgendaRepository implements AgendaRepository {
  ...
  // }
  // fix where
  async backfillAgenda(date: Date) {
  ...
  async base(data?: SelectAgendaQuery) {
    ...
    if (data?.withBackfillAgenda) {
      baseQuery = await this.backfillAgenda(data.date ?? new Date());
    }
    baseQuery = baseQuery
      .$if(data?.withCoach !== undefined, (qb): QueryResult<AgendaWithCoach> =>
        qb
          .innerJoin('coaches', 'agendas.coach_id', 'coaches.id')
          .innerJoin('users', 'coaches.user_id', 'users.id')
          .select(['users.name as coach_name', 'coaches.id as coach_id'])
      )
      .$if(data?.withClass !== undefined, (qb): QueryResult<AgendaWithClass> =>
        qb
      ...
      .$if(data?.withLocation !== undefined, (qb): QueryResult<AgendaWithLocation> =>
        qb
          .innerJoin(
            'location_facilities',
            'agendas.location_facility_id',
      ...
      .$if(data?.withAgendaBooking !== undefined, (qb): QueryResult<AgendaWithAgendaBooking> =>
        qb
          .innerJoin(
            'agenda_bookings',
            'agendas.id',
      ...
      .$if(data?.withParticipant !== undefined, (qb) =>
        qb.select((eb) =>
          eb
            .selectFrom('users')
      ...
      .$if(data?.withCountParticipant !== undefined, (qb) =>
        qb.select((eb) =>
          eb
            .selectFrom('agenda_bookings')
  ...
  async find<T extends SelectAgendaQuery>(data?: SelectAgendaQuery) {
  ...
  // async findByUserId(data: FindAgendaByUserOptions) {
  ...
  //             await this._creditRepository.delete({
  //               filters: {
